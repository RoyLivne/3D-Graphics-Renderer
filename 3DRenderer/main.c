	...
	while (SDL_PollEvent(&event)) {
		...
		case SDL_KEYDOWN:
			...
			if (event.key.keysym.sym == SDLK_UP) {
				update_camera_forward_velocity(vect3_mult(get_camera_direction(), 5.0 * delta_time));
				update_camera_position(vect3_add(get_camera_position(), get_camera_forward_velocity()));
				break;
			}
			if (event.key.keysym.sym == SDLK_DOWN) {
				update_camera_forward_velocity(vect3_mult(get_camera_direction(), 5.0 * delta_time));
				update_camera_position(vect3_subtract(get_camera_position(), get_camera_forward_velocity()));
				break;
			}
			break;
		}
...
void process_graphics_pipeline_stages(mesh_t* mesh) {
	...
	vect3_t up_direction = vec3_new(0, 1, 0);
	...
	for (int i = 0; i < num_faces; i++) {
		...
		//Caclaulate the triangle face normal
		...
		if (is_cull_backface()) {
			...
			vect3_t camera_ray = vect3_subtract(vec3_new(0, 0, 0), vec3_from_vec4(transformed_vertices[0]));
...
void render(void) {
	...
	for (int i = 0; i < num_triangles_to_render; i++) {
		...
		if (should_draw_vertex_points()) {
			draw_rectangle(triangle.points[0].x - 3, triangle.points[0].y - 3, 6, 6, 0xFF0000FF); // vertex A
			draw_rectangle(triangle.points[1].x - 3, triangle.points[1].y - 3, 6, 6, 0xFF0000FF); // vertex B
			draw_rectangle(triangle.points[2].x - 3, triangle.points[2].y - 3, 6, 6, 0xFF0000FF); // vertex C
		}
	}
